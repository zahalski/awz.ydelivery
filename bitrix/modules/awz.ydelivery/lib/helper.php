...
class Helper {
     ...
     * @throws SystemException
     */
    public static function getItems(Order $order, $profileId=0){

        if($profileId){
            $deliveryConfig = self::getConfigByProfileId($profileId);
        }else{
    ...
    public static function getConfigFromOrder(Order $order){
        ...
        if($order->getId() && isset(self::$cacheConfig['ORDER_'.$order->getId()])){

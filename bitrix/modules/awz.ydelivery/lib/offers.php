...
class OffersTable extends Entity\DataManager
{
     ...
     * @throws \Bitrix\Main\SystemException
     */
    public static function getPrepare(Order $order){
        ...
        if($checkMyDeliveryPvz){
            ...
            if(empty($prepareData['destination']['platform_station']['platform_id'])){
                $result->addError(new Error(Loc::getMessage('AWZ_YDELIVERY_OFFERS_ERR_NO_PVZ')));
                return $result;
            }

            if(empty($prepareData['source']['platform_station']['platform_id'])){
            ...
            if(empty($prepareData['destination']['custom_location']['details']['full_address'])){
